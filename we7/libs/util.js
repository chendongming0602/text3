...
function getSign(e, t, n) {
    ...
    f < i.length - 1 && (c += "&"));
    return n = n || _siteinfo2.default.token, o = r(c + n);
}
...
}, _apiConfig = require("../../config/api.config.js"), _apiConfig2 = _interopRequireDefault(_apiConfig), _siteinfo = require("../../siteinfo.js"), _siteinfo2 = _interopRequireDefault(_siteinfo), _base = require("base64"), _md = require("md5"), _md2 = _interopRequireDefault(_md), util = {};
...
util.base64Encode = function(e) {
    ...
    var n = _siteinfo2.default.siteroot + "?i=" + _siteinfo2.default.uniacid + "&gp=" + _apiConfig2.default.gp + "&t=" + _siteinfo2.default.multiid + "&v=" + _siteinfo2.default.version + "&from=wxapp&";
...
}, util.request = function(e, t) {
    var n, a = this, r = (require("underscore.js"), require("md5.js")), i = getApp();
    (e = e || {}).data = e.data ? e.data : {}, e.data.m = _apiConfig2.default.m, i && (e.data.version = i.globalData.version), 
    e.cachetime = e.cachetime ? e.cachetime : 0, e.showLoading = void 0 !== e.showLoading && e.showLoading;
    var o = wx.getStorageSync("userInfo").sessionid, s = e.url;
    if (-1 == s.indexOf("http://") && -1 == s.indexOf("https://") && (s = util.url(s)), 
    getUrlParam(s, "state") || e.data && e.data.state || !o || (s = s + "&state=we7sid-" + o), 
    !e.data || !e.data.m) {
        var u = getCurrentPages();
        u.length && (u = u[getCurrentPages().length - 1]) && u.__route__ && (s = s + "&m=" + u.__route__.split("/")[0]);
    }
    var c = getSign(s, e.data);
    if (c && (s = s + "&sign=" + c), !s) return !1;
    if (e.showLoading && util.showLoading(), e.cachetime) {
        var f = r(s), l = wx.getStorageSync(f), d = Date.parse(new Date());
        if (l && l.data) {
            if (l.expire > d) return e.complete && "function" == typeof e.complete && e.complete(l), 
            e.success && "function" == typeof e.success && e.success(l), console.log("cache:" + s), 
            wx.hideLoading(), wx.hideNavigationBarLoading(), !0;
            wx.removeStorageSync(f);
        }
    }
    if ("file" == t) return wx.uploadFile({
        url: e.url,
        filePath: e.filePath,
        name: "file",
        formData: e.formData,
        success: function(t) {
            t.data.error ? e.fail && options.fail(t) : e.success && options.success(t);
        },
        fail: function(t) {
            e.fail && options.fail(t);
        }
    }), !1;
    wx.request((n = {
        url: s,
        ...
        if (wx.hideNavigationBarLoading(), wx.hideLoading(), t.data.errno) {
            ...
            if (e.fail && "function" == typeof e.fail) e.fail(t); else if (t.data.message) {
                ...
                a.message(t.data.message, n, "error");
            }
            ...
            wx.setStorageSync(f, r);
        }
    }), _defineProperty(n, "fail", function(t) {
        wx.hideNavigationBarLoading(), wx.hideLoading();
        var n = require("md5.js")(s), a = wx.getStorageSync(n);
...
}, util.getUserInfo = function(e, t, n) {
    var a = function() {
    ...
    r.sessionid ? util.checkSession({
        ...
        fail: function() {
            r.sessionid = "", console.log("relogin"), wx.removeStorageSync("userInfo"), a();
        }
    }) : a();
    ...
    }, this.compareVersion = function(e, t) {
        e = (e + "").split("."), t = (t + "").split(".");
        for (var n = Math.max(e.length, t.length); e.length < n; ) e.push("0");
        for (;t.length < n; ) t.push("0");
        for (var a = 0; a < n; a++) {
            var r = parseInt(e[a]), i = parseInt(t[a]);
            if (r > i) return 1;
            if (r < i) return -1;
        }
        return 0;
